...
$log_file = "$output_dir\dvwa_low\dsss_dvwa.txt"
...
# Low Security
python $dsss_path -u "$url_sqli" --cookie="$cookie_low" > $log_file

Add-Content -Path $log_file -Value "`n"

python $dsss_path -u "$url_blind" --cookie="$cookie_low" >> $log_file